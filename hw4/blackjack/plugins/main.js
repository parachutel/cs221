...
function onLoad(assignmentId, ownerName, lastChanged, qaForumReleasePostUrl) {
  // Insert generic text
  ...
  if (qaForumReleasePostUrl !== undefined) {
    header.append($('<p>').append(
    '<b>Finally, before starting (and later, before submitting) we recommend you check out the <a href="' + qaForumReleasePostUrl + '" target="_blank">release Ed post</a> for any clarifications or updates.<b>'));
  }
  ...
  function addAltText(text) {
    return function(i, p) {
      $(p).prepend($("<span>", {class: "sr-only"}).append("(" + text + ") "));
    };
  }
  $('.writeup').each(addAltText("written"));
  $('.code').each(addAltText("code"));
