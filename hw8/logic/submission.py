...
def formula2b() -> Formula:
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return Exists('$p', Not(Exists('$c', Implies(Person('$p'), Child('$p', '$c')))))
    # END_YOUR_CODE
...
def formula2c() -> Formula:
    ...
    return Forall('$c', Forall('$f', Equiv(And(Male('$f'), Parent('$c', '$f')), Father('$c', '$f'))))
    # END_YOUR_CODE
...
def formula2d() -> Formula:
    ...
    return Forall('$gp', Forall('$gd', Equiv(
        And(Female('$gd'), Exists('$p', And(Child('$gp', '$p'), Child('$p', '$gd')))), 
        Granddaughter('$gp', '$gd'))))
    # END_YOUR_CODE
...
def liar() -> Tuple[List[Formula], Formula]:
    ...
    formulas.append(Exists('$honest', Forall('$suspect', 
        Equiv(TellTruth('$suspect'), Equals('$honest', '$suspect')))))
    formulas.append(Exists('$crasher', Forall('$suspect', 
        Equiv(CrashedServer('$suspect'), Equals('$crasher', '$suspect')))))
    # END_YOUR_CODE
...
def ints() -> Tuple[List[Formula], Formula]:
    ...
    formulas.append(Forall('$x', Exists('$s', And(Not(Equals('$x', '$s')), 
        Forall('$y', Equiv(Successor('$x','$y'), Equals('$s', '$y')))))))
    formulas.append(Forall('$x', And(Or(Even('$x'), Odd('$x')), Not(And(Even('$x'), Odd('$x'))))))
    formulas.append(Forall('$x', Forall('$s', Implies(
        And(Even('$x'), Successor('$x', '$s')), Odd('$s')))))
    formulas.append(Forall('$x', Forall('$s', Implies(
        And(Odd('$x'), Successor('$x', '$s')), Even('$s')))))
    formulas.append(Forall('$x', Forall('$s', Implies(
        Successor('$x', '$s'), Larger('$s', '$x')))))
