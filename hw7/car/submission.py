...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.numRows):
            for col in range(self.belief.numCols):
                otherX, otherY = util.colToX(col), util.rowToY(row)
                trueDist = math.sqrt((agentX - otherX) ** 2 + (agentY - otherY) ** 2) 
                pdf = util.pdf(trueDist, Const.SONAR_STD, observedDist)
                prob = self.belief.getProb(row, col) * pdf
    ...
    def elapseTime(self) -> None:
        ...
        for (oldTile, newTile), transProb in self.transProb.items():
            ...
            new_belief.addProb(*newTile, prob)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        re_weighted = collections.defaultdict(float)
        new_particles = collections.defaultdict(int)
        for tile in self.particles:
            otherX, otherY = util.colToX(tile[1]), util.rowToY(tile[0])
            trueDist = math.sqrt((agentX - otherX) ** 2 + (agentY - otherY) ** 2) 
            pdf = util.pdf(trueDist, Const.SONAR_STD, observedDist)
            re_weighted[tile] = self.particles[tile] * pdf
        for _ in range(self.NUM_PARTICLES):
            tile = util.weightedRandomChoice(re_weighted)
    ...
    def elapseTime(self) -> None:
        ...
        new_particles = collections.defaultdict(int)
        for old_tile, n_particles in self.particles.items():
            for _ in range(n_particles):
                possibleNewTiles = self.transProbDict[old_tile]
                newTile = util.weightedRandomChoice(possibleNewTiles)
