...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    constraints = lambda x, y: x[1] != y[1] and abs(x[0] - y[0]) != abs(x[1] - y[1])
    for i in range(n):
        csp.add_variable(variables[i], [(i, row) for row in range(n)]) # (col, row)
        [csp.add_binary_factor(variables[i], variables[j], constraints) 
...
class BacktrackingSearch:
    ...
    def get_unassigned_variable(self, assignment: Dict):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            mcvs = {}
            for var_id, var in enumerate(self.csp.variables):
                if var not in assignment:
                    n_consistent_vals = 0
                    for val in self.domains[var]:
                        if self.satisfies_constraints(assignment, var, val):
                            n_consistent_vals += 1
                    if n_consistent_vals not in mcvs:
                        mcvs[n_consistent_vals] = []
                    mcvs[n_consistent_vals].append(var_id)
            var_ids = mcvs[min(mcvs.keys())]
            return self.csp.variables[min(var_ids)]
            # END_YOUR_CODE
...
class SchedulingCSPConstructor:
    ...
    def add_quarter_constraints(self, csp: CSP) -> None:
        ...
        for request in self.profile.requests:
            for quarter in self.profile.quarters:
                if len(request.quarters) > 0: # If a request doesn't specify the quarter(s), do nothing.
                    csp.add_unary_factor((request, quarter),
                        lambda cid: True if quarter in request.quarters or \
                        cid is None else False)
        # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp: CSP) -> None:
        ...
        You should take advantage of create_sum_variable().
        ...
        #         by calling var = create_sum_var(csp, SOME_NAME, ['a', 'b'], 7).
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    quarter_course = (cid, quarter)
                    # Course unit
                    course_unit_domain = [0] + list(range(
                        self.bulletin.courses[cid].minUnits, 
                        self.bulletin.courses[cid].maxUnits + 1))
                    csp.add_variable(quarter_course, course_unit_domain)
                    quarter_plan.append(quarter_course)
                    # Consistency with requested course
                    csp.add_binary_factor((request, quarter), quarter_course, 
                        lambda request_cid, unit: unit > 0 if request_cid == cid 
                        else unit == 0)

            quarter_unit_sum_var = create_sum_variable(csp, f'q{quarter}_sum', 
                quarter_plan, self.profile.maxUnits)
            csp.add_unary_factor(quarter_unit_sum_var, 
                lambda unit_sum: self.profile.minUnits <= unit_sum <= self.profile.maxUnits)
